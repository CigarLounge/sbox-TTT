	...
	public class InventorySelection : Panel
	{
		...
		public override void Tick()
		{
			...
			foreach ( var slot in _entries.Values )
			{
				...
				slot.SetClass( "active", slot.Carriable.GetLibraryData().Title == activeItem?.GetLibraryData().Title );
				slot.SetClass( "opacity-heavy", slot.Carriable.GetLibraryData().Title == activeItem?.GetLibraryData().Title );
			...
			SortChildren( ( p1, p2 ) =>
			{
				...
				return result != 0
					? result
					: string.Compare( s1.Carriable.GetLibraryData().Title, s2.Carriable.GetLibraryData().Title, StringComparison.Ordinal );
			} );
		...
		private void ProcessClientInventorySelectionInput( InputBuilder input )
		{
				...
				for ( int i = 0; i < childrenList.Count; ++i )
				{
					if ( childrenList[i] is InventorySlot slot )
					{
						if ( (int)slot.Carriable.SlotType == keyboardIndexPressed )
						{
							...
							if ( slot.Carriable.GetLibraryData().Title == activeCarriable?.GetLibraryData().Title )
							{
			...
			if ( mouseWheelIndex != 0 )
			{
				int activeCarriableIndex = childrenList.FindIndex( ( p ) =>
					 p is InventorySlot slot && slot.Carriable.GetLibraryData().Title == activeCarriable?.GetLibraryData().Title );
		...
		private class InventorySlot : Panel
		{
			...
			public InventorySlot( Panel parent, ICarriableItem carriable ) : base( parent )
			{
				...
				Add.Label( carriable.GetLibraryData().Title );
